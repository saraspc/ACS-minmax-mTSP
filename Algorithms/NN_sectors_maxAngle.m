function [tour] = NN_sectors_maxAngle(problem)
% Partition the cities into sectors (based on angular positions)
% and apply the Nearest Neighbor (NN) heuristic for each traveler.


%% City partitioning based on angular sectors
cities_centroid=sectors_clustercities_maxAngle(problem);

%% NN heuristic implementation for each traveler
c0=problem.c0;
...
for k=1:m
    ...
    visited=ones(1,n);
    visited(cities_travelerk)=0
    visited(c0(k))=1;%depot (city 1) already visited
    while sum(visited)<n
        i=tour{k}(end);
        p=1./dist(i,:);
        p(isinf(p))=1;
...
title( strrep(name, '_', '\_')+ ", Nearest Neighbour sector heuristic  lmax=" +num2str(fbest_globalbest))
end
